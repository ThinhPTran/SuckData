...
;; ========================================================================== ;;
;;
;; this module allows reading some of the winglue database information for a
;; specific well.  This should include the stuff for modeling
;;
;; this is a work in progress.  It should be expanded to handle
;; additional data as there is a need for it.
;;
;; it is also the place to maintain the name-mappings between the
;; winglue database and how we want things in clojure

(ns winglue-well.database.core
  (:gen-class)
  (:use     [clojure.java.io])
  (:require [jdbc.core :as jdbc]
            ...
            [winglue-well.mimerefs :as mref]
            [winglue-well.utils :as util]
            [winglue-well.database.util :as du]
            [winglue-well.math :as math]
            [clojure.spec :as s]))

(defn dbspec [] (:dbspec @config/tao2-cfg))
...
(s/def ::dual string?)
(s/def ::name string?)
(s/def ::kb-elev #(instance? Double %))
(s/def ::cmpl-seq string?)
(s/def ::type string?)
(s/def ::jacket-name (s/nilable string?))
(s/def ::wellhead-md (s/nilable float?)) ;; not sure about this one
(s/def ::api-number string?)
(s/def ::cmpl #(or (string? %) (int? %)))
(s/def ::pwi int?)
(s/def ::well-master (s/keys :req-un [::lease
                                      ::dual
                                      ::name
                                      ::kb-elev
                                      ::cmpl-seq
...
(s/def ::point-list (s/and (s/coll-of number?) #(even? (count %))))

(s/def ::meas-depth-list ::point-list)
(s/def ::vert-depth-list ::point-list)
(s/def ::dsvy (s/keys :req-un [::meas-depth-list
                               ::vert-depth-list]))
